...
document.getElementById('ig-link').addEventListener('click', function(e) {
    ...
    // Deteksi apakah dibuka di dalam Browser WhatsApp / Instagram
    const isInsideApp = /FBAN|FBAV|Instagram|WhatsApp/i.test(navigator.userAgent);
    const isAndroid = /Android/i.test(navigator.userAgent);

    if (isAndroid && isInsideApp) {
        // Trik khusus Android: Paksa keluar dari browser internal WA ke Chrome
        window.location.href = intentUrl;
    } else {
        // Untuk iPhone atau Browser biasa (Chrome/Safari)
        window.location.href = appUrl;
        
        // Fallback kalau app gak kebuka
        setTimeout(() => {
